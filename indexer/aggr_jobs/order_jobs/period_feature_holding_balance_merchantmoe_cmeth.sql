...
WITH day_tokens AS (SELECT s.token_address,
     ...
     erc1155_balances AS (SELECT address, token_address, token_id, balance
                          ...
                          WHERE token_type = 'ERC1155'
                            and balance > 0),
     pools_token0_cmeth AS (SELECT p.token_address,
                                   p.token0_address,
                            ...
                            WHERE t0.symbol = 'cmETH'),
     pools_token1_cmeth AS (SELECT p.token_address,
                                   p.token0_address,
                                   t0.symbol   AS token0_symbol,
                                   t0.decimals AS token0_decimals,
                                   p.token1_address,
                            ...
                            FROM feature_merchant_moe_pools p
                                     JOIN tokens t0 ON t0.address = p.token0_address
                                     JOIN tokens t1 ON t1.address = p.token1_address
                            WHERE t1.symbol = 'cmETH'),
     pools_cmeth AS (SELECT *
                     FROM pools_token0_cmeth
                     UNION ALL
                     SELECT *
                     FROM pools_token1_cmeth)
